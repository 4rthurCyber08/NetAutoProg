...
#create/overwrite the call manager's info based on user's ip address
...
for i in range(1, 4):
    progress = "Configuring Ephones " + i + " /3"
...
for i in outgoing_calls_config:
    access_cli.send_config_set(outgoing_calls_config[i])
    print("Allowing Outgoing Calls for monitor number " + list_of_callers[i] + "..")
